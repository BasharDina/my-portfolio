...
function HeroObject({
  ...
  useFrame((state, delta) => {
    ...
    material.color.copy(baseColor).lerp(new THREE.Color("#02140d"), liveProgress);
    material.emissive.copy(new THREE.Color("#000000")).lerp(neonColor, liveProgress);
...
function Particles({
  ...
  useFrame((_, delta) => {
    ...
    materialRef.current.color.lerpColors(
      new THREE.Color("#9ae6b4"),
      new THREE.Color("#2fff9b"),
      liveProgress,
    );
  });
...
export default function ExperienceCanvas() {
  ...
  return (
    <div className="absolute inset-0" id="experience-pin">
      <Canvas camera={{ position: [0, 0, 3.2], fov: 45 }} dpr={[1, 1.6]}>
      ...
      <div
        className={`pointer-events-none absolute bottom-12 left-1/2 -translate-x-1/2 transition-all duration-500 ${
          ctaVisible ? "translate-y-0 opacity-100" : "translate-y-5 opacity-0"
        }`}
      >
        <a
          href="#contact"
          className="pointer-events-auto rounded-full border border-emerald-300/55 bg-emerald-400/20 px-6 py-3 text-sm font-semibold tracking-wide text-emerald-100 backdrop-blur-md transition hover:bg-emerald-300/25"
        >
          Start your project
        </a>
      </div>
