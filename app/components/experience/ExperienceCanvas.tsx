...
function Particles({
  ...
  useFrame((_, delta) => {
    ...
    const desiredCount = reducedMotion
      ? minCount
      : Math.round(minCount + (maxCount - minCount) * liveProgress);
...
function Scene({
  ...
  return (
    <>
      <fog attach="fog" args={["#07110b", 5.5, 15]} />
...
export default function ExperienceCanvas() {
  ...
  const dpr =
    typeof window !== "undefined"
      ? [1, Math.min(1.5, window.devicePixelRatio)] as [number, number]
  ...
  return (
    <div className="absolute inset-0" id="experience-pin">
      ...
      <div
        ...
        style={{
          background: `
            radial-gradient(circle at 70% 20%, rgba(47,255,155,0.10), transparent 45%),
            radial-gradient(circle at 30% 80%, rgba(255,255,255,0.04), transparent 40%)
          `,
        }}
      />
    </div>
